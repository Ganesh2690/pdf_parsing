...
def _build_html(
    ...
    return (
        ...
        + tab_analysis
        + "</div>\n"
        + '<div id="tab-scores" class="tab-panel hidden">'
        + tab_scores
        + "</div>\n"
        + '<div id="tab-texts" class="tab-panel hidden">'
        + tab_texts
        + "</div>\n"
        + '<div id="tab-recommendations" class="tab-panel hidden">'
        + tab_recommendations
        + "</div>\n"
        + _footer_with_script()
        + "\n</body>\n</html>"
...
def _nav_bar(winner: str, pdf_count: int, parser_count: int) -> str:
    return (
        ...
        + "</div>"
        + f'<span class="nav-meta">{parser_count} parsers · {pdf_count} PDFs · Winner: {winner}</span>'
        + "</nav>"
    )
...
def _tab_overview(
    ...
    return (
        ...
        f"<p>AI-powered comparison · Scored by GPT-4o</p>"
...
def _tab_scores(evaluations: list[PDFAIEvaluation]) -> str:
    ...
    header_cells = (
        "".join(f"<th style='color:#FFD740'>{p}</th>" for p in all_parsers)
        + "<th>Best</th>"
    )
...
def _deep_analysis_cards(evaluations: list[PDFAIEvaluation]) -> str:
    ...
    for ev in evaluations:
        ...
        for pname, ps in sorted(
            ev.parser_scores.items(), key=lambda x: -x[1].total_score
        ):
            c = _color(pname)
        ...
        out.append(
            ...
            f' <span style="font-size:.81rem;color:#888">({ev.pdf_type} · {ev.complexity})</span>'
...
def _tab_texts(
    ...
    for ev in evaluations:
        score_lookup[ev.pdf_id] = {
            p: ps.total_score for p, ps in ev.parser_scores.items()
        }
